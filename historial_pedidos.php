...
// Validar seguridad de trabajador/usuario general
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['cobrar_pedido'])) {
    ...
    $cobrado_por = $_SESSION['usuario']; // El trabajador que atiende la cuenta

    // Iniciar transacción segura
    $conexion->begin_transaction();

    try {
        // 1. Insertar en el historial de ventas
...
<head>
    ...
    <title>Pedidos Pendientes - Taquería Novillero</title>
    <style>
        body {
...
<body>
    ...
    <div class="container">
        ...
        <p>Monitoreo de cuentas activas en las mesas y barra (Vista Trabajador).</p>
                ...
                <?php if ($resultado && $resultado->num_rows > 0): ?>
                    <?php while ($fila = $resultado->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td style="text-align: center;">
                                <!-- Redirección interna corregida hacia historial_pedidos.php -->
                                <form action="historial_pedidos.php" method="POST" onsubmit="return confirm('¿Confirmar el cobro total de la MESA <?php echo $fila['mesa']; ?> por $<?php echo number_format($fila['total'], 2); ?>?');" style="margin:0;">
